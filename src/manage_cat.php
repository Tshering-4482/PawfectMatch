...
<html lang="en">
  <head>
    ...
    <title>Manage Cats|| Dashboard</title>
    ...
    <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
        <div class="position-sticky pt-3">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="manage_cat.php">
                ...
                <div class="pet-container">
                    ...
                    if ($result->num_rows > 0) {
                        while ($cat = $result->fetch_assoc()) {
                            ...
                            echo '<p>Sex: ' . htmlspecialchars($cat['sex']) . '</p>';
                            $image_path = '../images/' . htmlspecialchars($cat['image']);
                            echo '<img src="' . htmlspecialchars($cat['image']) . '" alt="Cat Image">';
                            echo '<div class="carousel-images">';

                            for ($i = 1; $i <= 8; $i++) {
                                $image_field = 'image_' . $i;
                                if (!empty($cat[$image_field])) {
                                    echo '<img src="' . htmlspecialchars($cat[$image_field]) . '" alt="Cat Image">';
                                }
                            }
                            echo '</div>';
                            echo '<form method="POST" action="delete_pet.php">';
                            echo '<input type="hidden" name="pet_id" value="' . $cat['cat_id'] . '">';
                            echo '<input type="hidden" name="pet_type" value="cat">';
                            echo '<button type="submit" class="delete-btn">Delete</button>';
                            echo '</form>';

                            echo '<a href="edit_pet.php?pet_id=' . htmlspecialchars($cat['cat_id']) . '&pet_type=cat">
                                    <button>Edit</button>
                            </a>';
                            echo '</div>';
                        }
